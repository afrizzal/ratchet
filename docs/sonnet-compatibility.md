# Sonnet Compatibility Rules for Ratchet
...
file or row deletion, or session/cookie handling** — it is risky. If it also lacks a bracket gate
(`[RISKY]`/`[OPS]`/`[USER-DECISION]`) *and* you cannot run its acceptance criteria to green in this
environment, **stop and set it `needs-human`** with note "risky, unverifiable here". Do not implement it.
...
## 8. Handle failing tests as diagnosis, bounded to 3 attempts
Red → read the actual failure, form one hypothesis, change one thing, re-run. Journal one line per
attempt (what failed, what you changed). After 3 attempts without green: revert the item's source
changes only, set `blocked`, Journal the best hypothesis and what a human should look at. Do not thrash
past 3. Do not "sort of" pass — a flaky check that "basically works" has not passed.
...
ledger row flipped to `done`. Nothing else in the diff. Stage the specific paths (`git add <src>
ratchet/BACKLOG.md`), confirm the staged diff is only those files, then commit with the repo's message
convention (`ratchet(<ID>): <title>` if none detected). Never batch several items into one commit —
per-item revertability is the whole point.

## 12. Stop instead of improvising
Stop and hand back to a human when: two items block in a row (something systemic is wrong); the Spec
contradicts the code; an eligible item trips rule 1 or 4; or context runs low (write state, commit,
report the resume command). Stopping cleanly with the state written is a *success*. Improvising past an
ambiguity — guessing what a stale item meant, forcing a red check green, expanding scope "because the
real problem is bigger" — is the failure mode Ratchet exists to prevent. When unsure, park it.
