...
2. **Require a clean working tree.** `git status` must show no staged or unstaged changes to tracked files. Dirty → STOP and ask the user (their work is not yours to stash). This invariant is what makes your failure-recovery safe. Exception: the backlog file and its `ratchet/` directory are the loop's *own* state — if they are untracked, that is not user dirt; commit them first (`ratchet: add backlog`) so the tree is clean and your per-item commits can update a tracked ledger.
...
**High-stakes correctness gate.** An item can be technically ungated yet still be one where a subtly wrong implementation corrupts money, GL balances, tenant isolation, or auth — and where the acceptance tests might pass while the behavior is wrong. Treat any item that (a) is P0/HIGH **and** (b) touches money/GL, authorization, tenant boundaries, migrations, or data deletion as requiring `--verify fresh`. In an unattended run or when you are a smaller/cheaper model, if `--verify fresh` is not in effect, park such an item as `needs-human` (note: "high-stakes; needs clean-room verify") rather than soloing it. The cost of a wrong auth/money commit that looks green is far higher than the cost of a human glance.
...
6. **Attempts exhausted → block cleanly.** Revert the item's *source* changes, but **never revert or delete the backlog file or the `ratchet/` directory** — that is the loop's own state and it must survive to record the block. So: `git restore --source=HEAD --worktree -- <the source paths this item touched>` (enumerate them; do **not** `git checkout -- .`, which would also wipe your in-progress ledger edits), then remove only the source files this item created — explicitly, by name. Avoid a blanket `git clean -fd`: if `ratchet/` isn't committed yet it will delete your audit and backlog. Confirm `git status` shows only the backlog file modified (your ledger/journal writes), then set Ledger `blocked` + attempts count; Journal: what failed, best hypothesis, what a human should look at. Commit the ledger/journal update so the block is durable.
